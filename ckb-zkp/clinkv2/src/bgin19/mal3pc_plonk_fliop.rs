...
fn bgin19_mul_plonk_fliop() {
    use ark_serialize::*;

    let T: usize = 100000; // 100000(k=64): 19s; 10000(k=64): 1.3s (997bytes) 10000(k=64): 1.15s (1383bytes) 
    let k: usize = 8; 
    let n = 80;
    let L = 4 * n + 3;
    ...
    let proof = prove_dot_prod::<Bls12_381, _>(&inputs, &rhos, k, &kzg10_ck, sid, rng);
    let prove_time = prove_start.elapsed();
    println!("Proving time: {:?}", prove_time);

    let mut proof_bytes = vec![];
    proof.serialize(&mut proof_bytes).unwrap();
    println!("[PLONK FLIOP] Proof length: {}", proof_bytes.len());
...
fn sh3pc_dotprod() {
    ...
    let T: usize = 3000000; // 100000(k=64): 19s; 10000(k=64): 1.3s (997bytes) 10000(k=64): 1.15s (1383bytes) 
